    ...
    <div class="basis-2/12 min-h-[755px] flex flex-col justify-between card border">
      <div class="flex gap-4 flex-col items-center">
        <div class="relative">
          ...
          <i
            @click="OpenFile()"
            class="pi pi-pencil absolute bottom-[30px] right-[6px] text-green-600 bg-white border-green-200 border p-2 rounded-full cursor-pointer hover:text-white hover:bg-green-400"
          ></i>
        </div>
          ...
          <TabPanel value="0">
                ...
                <div class="basis-1/2 flex flex-col gap-4">
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <Textarea
                      rows="4"
                      class="w-full"
                      :placeholder="`${dataUser.address}, ${dataUser.ward}, ${dataUser.district}, ${dataUser.province}`"
                      readonly
                    ></Textarea>
                  </div>
                </div>
            ...
            >
              ...
              <div class="flex gap-2 mb-6">
                <Select
                  ...
                  :options="cities"
                  option-label="name"
                  :placeholder="dataUser.province"
                  @change="onChangeCities"
                  class="w-full"
                ></Select>
                <Select
                  v-model="selectedDistrict"
                  :placeholder="dataUser.district"
                  :options="districts"
            ...
            <div class="card border">
              ...
              <div class="flex gap-8">
                <div class="basis-1/2 flex flex-col gap-6">
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <InputText
                      v-model="PayloadCP.newPasswordAgain"
                      type="password"
                      class="w-full"
                    ></InputText>
                  </div>
              ...
              <div class="flex justify-end">
                <Button class="px-4 py-2 text-white rounded-md" @click="updatePassword()"
...
const onChangeCities = async () => {
  const res = await API.get(`address/district/${selectedCity.value.id}`);
  districts.value = res.data.metadata;
};

const onChangeDistrict = async () => {
  const res = await API.get(`address/ward/${selectedDistrict.value.id}`);
...
const updatePassword = async () => {
  const validate = validatePassword(
    PayloadCP.value.newPassword,
    PayloadCP.value.newPasswordAgain
  );
  if (validate !== 0) {
...
const updateData = async () => {
  if (!selectedCity.value || !selectedDistrict.value || !selectedWards.value) {
    toast.add({
      severity: "error",
      summary: "Cập nhật thất bại",
      detail: "Vui lòng nhập đầy đủ trường",
      life: 3000,
    });
    return;
  }
  ...
  payload.value.address = dataUser.value.address;
  payload.value.province = selectedCity.value.name;
  payload.value.district = selectedDistrict.value.name;
  payload.value.ward = selectedWards.value.name;
