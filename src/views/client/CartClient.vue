<template>
  ...
  <div class="my-5">
    <Button v-permission="'admin'" label="Toi ten la hoang"></Button>
    ...
    <div class="card mt-5">
      <DataTable v-model:selection="selectedProduct" show-gridlines :value="payload">
        ...
        <Column header="Thông tin sản phẩm" field="book._id" style="max-width: 300px">
          <template #body="{ data }">
            <div class="flex items-center justify-between">
              <router-link :to="{ name: 'DetailBook', params: { slug: data.book.slug } }">
                <div class="flex items-center gap-5">
                  <img
                    ...
                    :src="data.book.imageBook"
                    alt=""
                    class="w-[80px] h-[120px] hover:shadow-md"
                  />
              ...
              <span
                class="block text-red-500 text-sm hover:underline cursor-pointer"
                @click="Delete(data.book._id)"
              >
                Xóa
              </span>
            </div>
        ...
        <Column header="Đơn giá" field="book.price">
          <template #body="{ data }">
            {{
              currency(data.book.price * ((100 - data.book.discount) / 100), {
                symbol: "đ",
                separator: ",",
              }).format()
            }}</template
        ...
        <Column header="Số lượng" field="quantity" style="max-width: 100px">
          <template #body="{ data, index }">
            <InputGroup>
              <Button
                icon="pi pi-minus"
                @click="ClickQuantity('m', data, index)"
                severity="danger"
                outlined
              />
              ...
              <Button
                icon="pi pi-plus"
                @click="ClickQuantity('a', data, index)"
                severity="info"
                outlined
              />
            </InputGroup>
            <div class="text-right">
              <span class="text-sm text-gray-400 italic text-right">
                Số lượng trong kho: {{ data.book.quantity }}
              </span>
            </div>
          </template>
        </Column>
        <Column header="Thành tiền">
          <template #body="{ data }">
            {{
              currency(
                data.book.price * ((100 - data.book.discount) / 100) * data.quantity,
                { symbol: "đ", separator: "," }
              ).format()
            }}</template
          >
...
const Update = () => {
  ...
  for (let index = 0; index < payload.value.length; index++) {
    ...
    item.price =
      payload.value[index].book.price *
      ((100 - payload.value[index].book.discount) / 100) *
      payload.value[index].quantity;
    if (item.quantity !== 0) {
  ...
  if (action == "m") {
    if (data.quantity >= 2) {
      ...
      toast.add({
        severity: "error",
        summary: "Lỗi số lượng",
        detail: "Số lượng quá ít. Vui lòng nhập số lượng >= 1",
        life: 3000,
      });
    }
  }
