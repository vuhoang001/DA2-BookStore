        ...
        <div class="flex flex-col gap-5">
          ...
          <div>
            ...
            <router-link
              :to="{
                name: 'AuthorClient',
                params: { slug: DetailBook.authorBook?.slug },
              }"
            >
              <span
                class="uppercase font-semibold hover:cursor-pointer hover:underline"
                >{{ DetailBook.authorBook?.authorName }}</span
              >
            </router-link>
          </div>
          <div class="flex gap-4 items-center">
            <!-- {{ book.discount }} -->
            <span v-if="DetailBook.discount !== 0" class="italic line-through">{{
              formatCurrency(DetailBook.price)
            }}</span>
            <span class="text-2xl">{{
              formatCurrency(DetailBook.price * ((100 - DetailBook.discount) / 100))
            }}</span>
            <Button
              v-if="DetailBook.discount !== 0"
              :label="DetailBook.discount + ' %'"
              severity="danger"
            ></Button>
          </div>
          <Divider></Divider>
          ...
          <div class="flex gap-10 items-center">
            <div class="basis-4/12">
              <InputGroup>
                ...
                <Button
                  icon="pi pi-plus"
                  @click="ClickQuantity('a')"
                  severity="info"
                  outlined
                />
              </InputGroup>
            </div>
            <div class="basis-auto italic text-gray-400">
              Còn lại {{ DetailBook.quantity }} trong kho
            </div>
          </div>
...
const formatDate = (date) => {
  ...
  const month = String(d.getMonth() + 1).padStart(2, "0"); // Đảm bảo tháng có 2 chữ số
...
const addCart = async (detailBook, action) => {
  ...
  try {
    const total =
      detailBook.price * ((100 - detailBook.discount) / 100) * payload.value.quantity;
    store
      .dispatch("cart/addToCart", [
        { productId: detailBook._id, quantity: payload.value.quantity, price: total },
      ])
      .then((res) => {
        if (res) {
...
const author = {
  ...
  text:
    "Nếu cầu muốn một iều gì đó lớn, cả vũ trụ sẽ hin thc giúp bạn điều đó, qua những dấu hiệu mà nếu cầu nhìn kỹ mới có thể nhận ra.",
