            ...
            <div class="flex flex-col gap-5 basis-4/6 card">
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex items-center border-2 gap-3 p-3 rounded-md">
                        <RadioButton v-model="payload.paymentMethod" input-id="cod" value="C"></RadioButton>
                        <img src="../../images/money-svgrepo-com.svg" alt="" class="w-[50px] h-[50px] ml-4" />
                    ...
                    <div class="flex items-center gap-3 border-2 p-3 rounded-md">
                        <RadioButton v-model="payload.paymentMethod" input-id="zalo" value="T"></RadioButton>
                        <img src="../../images/zalopay-svgrepo-com.svg" alt="" class="w-[50px] h-[50px] ml-4" />
            ...
            <div class="basis-2/6 card">
                <div v-for="item in orderData" class="flex items-center justify-between mt-4">
                    <div class="flex items-center gap-7 relative">
                        <img crossorigin="anonymous" :src="item.book.imageBook" class="w-[70px] h-[100px] object-cover" alt="" />
                        <span class="text-base">
                            {{ item.book.bookName }}
                        </span>

                        <div class="absolute top-[-10px] left-[68px] bg-gray-400 text-white text-xs px-2 py-1 rounded-full">
                            {{ item.quantity }}
                        </div>
                    </div>
                ...
                <div class="flex gap-5">
                    <div class="basis-4/6">
                        <InputText v-model="payload.discord" placeholder="Mã giảm giá" class="w-full"></InputText>
                    </div>
                    <div class="basis-2/6">
                        <Button label="Sử dụng" class="w-full" :disabled="payload.discord !== '' ? false : true"></Button>
                    </div>
                </div>
                <Divider></Divider>
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-xl">{{ currency(payload.totalAmount, { symbol: 'đ', separator: ',' }).format() }}</span>
                </div>
