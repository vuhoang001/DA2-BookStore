<template>
    ...
    <div class="card mt-5">
        <DataTable :value="products" paginator :rows="5" :rows-per-page-options="[5, 10, 15, 20]" show-gridlines>
            <template #header>
    ...
    <Dialog v-model:visible="visibleDialog" modal header="Tạo khuyến mãi" style="width: 30%">
        <Tabs value="0">
            <TabList>
                ...
                <TabPanel value="0">
                    <div class="flex flex-col gap-4">
                        <div class="flex flex-col gap-2">
                            ...
                            <MultiSelect v-model="payload.selectedBooks" display="chip" :options="products" option-label="bookName" filter class="w-full">
                                <template #option="{ option }">
                                    <div class="flex justify-between items-center">
                                        <img crossorigin="anonymous" class="w-14 h-22" :src="option.imageBook" alt="" />
                                        <div class="flex flex-col justify-between ml-10">
                                            <span class="text-lg font-medium">{{ option.bookName }}</span>
                                            <span class="text-gray-500 italic text-sm">{{ currency(option.price, { symbol: 'đ', separator: ',' }).format() }}</span>
                                        </div>
                                    </div>
                                </template>
                                ...
                                <template #footer>
                                    <div class="text-right mr-4">
                                        <Button severity="danger" @click="removeAll('B')" class="hover:underline cursor-pointer" label="Xóa tất cả" text></Button>
                                    </div>
                                </template>
                            </MultiSelect>
                        </div>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <InputNumber v-model="payload.discount" inputId="horizontal-buttons" showButtons buttonLayout="horizontal" max="100" min="0" :step="5" fluid>
                ...
                <TabPanel value="1">
                    <div class="flex flex-col gap-4">
                        <div class="flex flex-col gap-2">
                            ...
                            <MultiSelect v-model="payload.selectedGenre" display="chip" :options="genre" option-label="genreName" filter class="w-full">
                                <template #option="{ option }">
                                    <div class="flex justify-between items-center">
                                ...
                                <template #footer>
                                    <div class="text-right mr-4">
                                        <Button severity="danger" @click="removeAll('G')" class="hover:underline cursor-pointer" label="Xóa tất cả" text></Button>
                                    </div>
                                </template>
                            </MultiSelect>
                        </div>
...
const UpdateDiscount = async (action) => {
    let type = action;
    let ids = [];
    const array = type == 'group' ? payload.value.selectedBooks : payload.value.selectedGenre;
    for (var item of array) {
        ids.push(item._id);
    }

    try {
        const res = await API.create(`book/discount?type=${type}`, { ids: ids, percent: payload.value.discount });
